...
export default function Statistics() {
  ...
  const [checkbox_Hold_State, set_checkbox_Hold_State] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  useEffect(() => {
    const pchecks = window.localStorage.getItem("local_pchecks");
    if (pchecks) {
      set_checkbox_Hold_State(JSON.parse(pchecks));
    }
    ...
    // const extendedTallyValue = window.localStorage.getItem("Extended_Tally_Value");
    //       if (extendedTallyValue !== null) setExtendedTallyValue(JSON.parse(extendedTallyValue));
  ...
  useEffect(() => {
    window.localStorage.setItem("Extended_Tally_Value", JSON.stringify(Extended_Tally_Value));
    window.localStorage.setItem("local_pchecks", JSON.stringify(checkbox_Hold_State));
    window.localStorage.setItem("local_punits", JSON.stringify(previous_Value_QT));
    window.localStorage.setItem("local_pkilos", JSON.stringify(previous_Value_TO));
    window.localStorage.setItem("local_pmaxis", JSON.stringify(maxi_Value_TO));
}, [Extended_Tally_Value, checkbox_Hold_State, previous_Value_QT, previous_Value_TO, maxi_Value_TO]);  
...
// Fonction pour gérer le changement de Extended_Tally_Value
  ...
  const totalPreviousCalesCount = Object.keys(previous_Value_QT).reduce(
    (total, k) => {
      return (
        total + (previous_Value_QT[k] ? parseFloat(previous_Value_QT[k]) : 0)
      );
    },
    0
  );
  ...
  const totalPreviousCalesWeight = Object.keys(previous_Value_TO).reduce(
    (total, k) => {
      return (
        total +
        (previous_Value_TO[k]
          ? parseFloat(previous_Value_TO[k])
          : 0)
      );
    },
    0
      ...
      <Table>
        ...
        <tbody>
                ...
                <tr key={affectationItem.name}>
                  ...
                  >
                    ...
                    {affectationItem.name !== "stock" ? (
                      <input
                        ...
                        onChange={() =>
                          Toggle_checkbox_boolean(affectationItem.name)
                        }
                      />
                    ) : null}
                  </td>
                  ...
                  <td style={{ textAlign: "center" }}>
                    {statistics[affectationItem.name]
                      ? statistics[affectationItem.name].count
                      : 0}
                  </td>
                  {/* </tr> */}
                  {/* DAY_T */}
          ...
          <tr>
            ...
            <td style={{ textAlign: "center" }}>
              {isNaN(totalCalesCount) ? 0 : totalCalesCount}
            </td>
            {/* T */}
            <td style={{ textAlign: "center" }}>
              {isNaN(totalCalesWeight)
                ? 0
                : totalCalesWeight.toLocaleString("en-US")}
            </td>
            {Extended_Tally_Value && (
              <>
                {/*PREV_Q */}
                <td style={{ textAlign: "center", backgroundColor: "gray" }}>
                  {isNaN(totalPreviousCalesCount) ? 0 : totalPreviousCalesCount}
                </td>
                {/*PREV_T */}
                <td style={{ textAlign: "center", backgroundColor: "gray" }}>
                  {isNaN(totalPreviousCalesWeight)
                    ? 0
                    : totalPreviousCalesWeight.toLocaleString("en-US")}
                </td>
              </>
            )}
            {/* TT_Q */}
            <td style={{ textAlign: "center" }}>
              {totalCalesCount + totalPreviousCalesCount}
            </td>
            {/* TT_T */}
            <td style={{ textAlign: "center" }}>
              {(totalCalesWeight + totalPreviousCalesWeight).toLocaleString(
                "en-US"
              )}
            </td>
          </tr>
                    ...
                    {affectationItem.name === "stock" ? (
                      ...
                      <>
                        ...
                        <td
                          ...
                          className={
                            parseFloat(
                              maxi_Value_TO[affectationItem.name] ?? 0
                            ) -
                              ((parseFloat(
                                statistics[affectationItem.name]?.weight
                              ) || 0) +
                        ...
                        <td
                          ...
                          className={(() => {
                            try {
                              ...
                              const result =
                                (maxiTo - statsWeight - prevTO) /
                                ((statsWeight + prevTO) /
                                  (statistics[affectationItem.name]?.count +
                                    prevQT_VALUE));

                              return Math.floor(isNaN(result) ? 0 : result) < 0
                                ? "red-text"
                                : "blue-text";
                            } catch (error) {
                              return "red-text";
