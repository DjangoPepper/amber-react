...
interface DataState {
	...
	HOLD_checked_BOX:    { [destination: string]: boolean };
...
*/
			case DataAction.CHANGE_CHECKBOX_STATE:
				...
				// Vérifiez si le couple existe déjà
...
////		
				case DataAction.CREATE_CHECKBOX_STATE:
					// Assurez
					const {destinatioCHECKBOX, boolCHECKBOX } = action.payload;
					...
					return {
						...
						HOLD_checked_BOX: {
							...state.HOLD_checked_BOX,
							[destinatioCHECKBOX]: boolCHECKBOX,
						},
					};


				case DataAction.CREATE_PREVIOUS_QTT_STATE:
				// Assurez-vous que l'action a des propriétés payload avec les valeurs nécessaires
					const { destinationQTT, valueQTT } = action.payload;

					// Ajoutez le nouveau couple key:value dans HOLD_previous_QTT
					return {
						...state,
						HOLD_previous_QTT: {
							...state.HOLD_previous_QTT,
							[destinationQTT]: valueQTT,
						},
					};

			case DataAction.CREATE_PREVIOUS_TONS_STATE:
				// Assurez-vous que l'action a des propriétés payload avec les valeurs nécessaires
				const { destinationTONS, valueTONS } = action.payload;
				...
				return {
					...
					HOLD_previous_TONS: {
						...state.HOLD_previous_TONS,
						[destinationTONS]: valueTONS,
					},
				};
			case DataAction.CREATE_MAXI_TONS_STATE:
				// Assurezz vous
				const {destinationMAXITONS, valueMAXITONS } = action.payload;
				...
				return {
					...
					HOLD_maxi_TONS: {
						...state.HOLD_maxi_TONS,
						[destinationMAXITONS]: valueMAXITONS,
					},
				};
...
//
			...
			case DataAction.SAVE_CHECKBOX_STORAGE:
				// if(state.saved_HOLD_checked_box) return state;
				window.localStorage.setItem("CHECKBOX_data_storage", JSON.stringify(state.HOLD_checked_BOX));
				return {
					...state,
					saved_HOLD_checked_box: false
				};
			case DataAction.SAVE_PREV_QTT_STORAGE:
			...
			case DataAction.SAVE_PREV_TONS_STORAGE:
			...
			case DataAction.SAVE_MAXI_TONS_STORAGE:
...
//
			...
			case DataAction.LOAD_CHECKBOX_STATE:
				return {
					...state,
					HOLD_checked_BOX: action.payload,
					loaded_HOLD_checked_box: true,
					saved_HOLD_checked_box: false,
				};
