...
export const dataReducer: Reducer<Interface_stepe_state> = (
  state = initial_stepe_Data_State,
  action: AnyAction
): Interface_stepe_state => {
  switch (action.type) {
    ...
    case DataAction.MOVE_ROW:
      const d = state.catalog_data_state.find(
        (r) => r.reference === action.payload
      );
      if (!d) return state;
      return {
        ...state,
        catalog_data_state: [
          ...state.catalog_data_state.filter(
            (r) => r.reference !== action.payload
          ),
          { ...d, destination: state.selectedCale },
        ],
        saved_catalog_status: false,
      };
    case DataAction.UPDATE_ROW:
      const currentRow = state.catalog_data_state.find(
        (r) => r.reference === action.payload.reference
      );
    ...
    case DataAction.SAVE_CATALOG:
      ...
      window.localStorage.setItem(
        "local_catalog",
        JSON.stringify(state.catalog_data_state)
      );
      return {
        ...state,
        saved_catalog_status: false,
    ...
    case DataAction.SAVE_TALLY:
      ...
      window.localStorage.setItem(
        "local_tally",
        JSON.stringify(state.tally_data_state)
      );
      return {
        ...state,
    ...
    case DataAction.SAVE_CHECKBOX_STATE:
      ...
      window.localStorage.setItem(
        "local_checkbox",
        JSON.stringify(state.TAlly_HOLD_checkbox)
      );
    ...
    case DataAction.SAVE_PREV_QTT:
      ...
      window.localStorage.setItem(
        "local_punits",
        JSON.stringify(state.TAlly_HOLD_previous_QTT)
      );
      return {
        ...state,
        saved_TAlly_HOLD_previous_QTT_status: false,
    ...
    case DataAction.SAVE_PREV_TONS:
      ...
      window.localStorage.setItem(
        "local_pkilos",
        JSON.stringify(state.TAlly_HOLD_previous_TONS)
      );
      return {
        ...state,
        saved_TAlly_HOLD_previous_TONS_status: false,
    ...
    case DataAction.SAVE_MAXI_TONS:
      ...
      window.localStorage.setItem(
        "local_maxis",
        JSON.stringify(state.TAlly_HOLD_maxi_TONS)
      );
      return {
        ...state,
        saved_TAlly_HOLD_maxi_TONS_status: false,
